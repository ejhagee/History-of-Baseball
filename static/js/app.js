/* 
function buildAllTimeWinPctCharts(team) {
    
    // Connect to flask
    var url = "/teams/winpct";
    var location = document.getElementById('chart-1');
    d3.json(url).then(function(data) {

      console.log(data);

      console.log(data.FranchiseName);
...
function buildWinPrctCharts(team) {


  
      // Bar Graph Using Plotly
      url2 = "/teams/${team}"
      location2 = document.getElementById('chart-2');
      d3.json(url2).then(function(data) {
        ...
        var data = [{
          ...
          text:["Win World Series: " + WinWS,
                ...
                "Wins: " + W,
                "Losses: " + L,
                "Win Percentage: " +  yValues]

        }];
        // initiate layout
        var layout = {title: "Win Percentage to Year for: " + FranchiseName,
                      showlegend: false,
                      xaxis: {tickangle: -90},
                      bargap: 0.05
        };
...
function buildBestHitterChart(team, bat_stat)
{
  ...
  var location = document.getElementById('chart-3');
  d3.json(url).then(function(data){
...
function init() {
  ...
  //build first charts
  //window.setTimeout(buildChartsPlottable(firstTeam), 10000);
  window.setTimeout(buildHittingChart(firstTeam, firstBatStat), 1000);
  window.setTimeout(buildPitchingChart(firstTeam, firstPitchStat), 30000);
...
function teamChanged(team) {
  ...
  //build new charts
  buildChartsPlottable(team);
  buildHittingChart(team, firstBatStat);
  buildPitchingChart(team, firstPitchStat);
}
